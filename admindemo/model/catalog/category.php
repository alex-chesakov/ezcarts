...
class ModelCatalogCategory extends Model {
	...
	public function getCategories($data) {
		$sql = "SELECT category_id, GROUP_CONCAT(name SEPARATOR ' &gt; ') AS name, parent_id, sort_order FROM " . DB_PREFIX . "category WHERE 1=1";
		if (!empty($data['filter_name'])) {
			$sql .= " AND name LIKE '" . $this->db->escape($data['filter_name']) . "%'";
		}
		$sql .= " GROUP BY category_id ORDER BY name";

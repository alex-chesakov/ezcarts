...
class ModelSaleOrder extends Model {
	...
	public function getOrder($order_id,$user_id = 0) {
		$sql = "SELECT *, (SELECT c.firstname FROM " . DB_PREFIX . "customer c WHERE c.customer_id = o.customer_id) AS customer FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'";
		if(!empty($user_id)){
			$sql.= " AND o.user_id = '" . (int)$user_id . "'";
		}
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'user_id' => (int)$order_query->row['user_id'],
				'customer_id'             => $order_query->row['customer_id'],
				'customer'                => $order_query->row['customer'],
				'firstname'               => $order_query->row['firstname'],
				'email'                   => $order_query->row['email'],
				'telephone'               => $order_query->row['telephone'],
				'date_shipping' => $order_query->row['date_shipping'],
				'time_shipping' => $order_query->row['time_shipping'],
				'unavailable' => $order_query->row['unavailable'],
				'collector' => $order_query->row['collector'],
				'kitchen_checkout' => (int)$order_query->row['kitchen_checkout'],
				'payment_method' => $order_query->row['payment_method'],
				'comment' => $order_query->row['comment'],
				'collection_window' => $order_query->row['collection_window'],
				'total'                   => $order_query->row['total'],
				'order_status_id' => (int)$order_query->row['order_status_id'],
				'payment_status_id' => (int)$order_query->row['payment_status_id'],
				'ip'                      => $order_query->row['ip'],
				'user_agent'              => $order_query->row['user_agent'],
'req'              => $order_query->row['req'],	
'proof'              => $order_query->row['proof'],	
'timer' => $order_query->row['timer'],				
	...
	public function getOrders($data = array()) {
		$sql = "SELECT ps.payment_status, o.order_status_id, o.user_id, o.order_id, op.location, o.firstname AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id) AS status, o.total, o.date_added, o.date_modified, o.date_shipping, o.time_shipping FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_product op ON (o.order_id = op.order_id) LEFT JOIN " . DB_PREFIX . "payment_status ps ON (o.payment_status_id = ps.payment_status_id) WHERE 1=1 ";

		if (isset($data['filter_location']) && !is_null($data['filter_location'])) { $sql .= " AND op.location LIKE '" . $this->db->escape($data['filter_location']) . "'";}
		if (isset($data['filter_user_id']) && !is_null($data['filter_user_id'])) { $sql .= " AND o.user_id LIKE '" . (int)$data['filter_user_id'] . "'";}
		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {$sql .= " AND o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";}
		if (isset($data['filter_payment_status_id']) && !is_null($data['filter_payment_status_id'])) {$sql .= " AND o.payment_status_id = '" . (int)$data['filter_payment_status_id'] . "'";}
		if (!empty($data['filter_order_date'])) {$sql .= " AND DATE(o.date_added) = DATE('" . $this->db->escape($data['filter_order_date']) . "')";}
		if (!empty($data['filter_delivery_date'])) {$sql .= " AND DATE(o.date_shipping) = DATE('" . $this->db->escape($data['filter_delivery_date']) . "')";}
		
		if (!empty($data['filter_name'])) {
			$sql.= " AND (";
			$sql .= " o.firstname LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
			
			$f = preg_replace('/[^0-9]/', '', $data['filter_name']);
			$f = (int)$f;
			if(!empty($f)){
				$sql .= " OR o.order_id = '" . (int)$f . "'";
			}
			
			$sql .= " OR o.email LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
			$sql .= " OR o.telephone LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
			$sql .= " OR op.location LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
...
//	echo $sql;
	...
	public function getTotalOrders($data) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_product op ON (o.order_id = op.order_id) WHERE 1=1 ";
		if (isset($data['filter_location']) && !is_null($data['filter_location'])) { $sql .= " AND op.location LIKE '" . $this->db->escape($data['filter_location']) . "'";}
		if (isset($data['filter_user_id']) && !is_null($data['filter_user_id'])) { $sql .= " AND o.user_id LIKE '" . (int)$data['filter_user_id'] . "'";}
		if(!empty($data['filter_order_status_id'])){$sql.= "AND order_status_id = '" . (int)$data['filter_order_status_id'] . "' ";}
		if (isset($data['filter_payment_status_id']) && !is_null($data['filter_payment_status_id'])) {$sql .= " AND o.payment_status_id = '" . (int)$data['filter_payment_status_id'] . "'";}
		if (!empty($data['filter_order_date'])) {$sql .= " AND DATE(o.date_added) = DATE('" . $this->db->escape($data['filter_order_date']) . "')";}
		if (!empty($data['filter_delivery_date'])) {$sql .= " AND DATE(o.date_shipping) = DATE('" . $this->db->escape($data['filter_delivery_date']) . "')";}
		if (!empty($data['filter_name'])) {
...
//		if(!empty($data['filter_customer'])){$sql.= "AND customer_id = '" . (int)$data['filter_customer'] . "' ";}	
//		if(!empty($data['filter_total'])){$sql.= "AND total = '" . (int)$data['filter_total'] . "' ";}
		$sql.= " GROUP BY o.order_id ";
...
/*	+ */
	public function getOrderProducts($order_id) { 
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "' ORDER BY bin + 0 ASC");
		return $query->rows;
	}

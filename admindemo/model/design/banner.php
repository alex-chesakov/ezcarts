...
class ModelDesignBanner extends Model {
	public function addBanner($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "banner SET name = '" . $this->db->escape($data['name']) . "', status = '" . (int)$data['status'] . "', `filename` = '" .  $this->db->escape($data['filename']) . "'");
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				...
				foreach ($banner_image['banner_image_description'] as $language_id => $banner_image_description) {				
					$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image_description SET banner_image_id = '" . (int)$banner_image_id . "', language_id = '" . (int)$language_id . "', banner_id = '" . (int)$banner_id . "', title = '" .  $this->db->escape($banner_image_description['title']) . "', html_desc = '" .  $this->db->escape($banner_image_description['html_desc']) . "'");
				}
			}
		}		
	}
	...
	public function editBanner($banner_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "banner SET name = '" . $this->db->escape($data['name']) . "', status = '" . (int)$data['status'] . "', `filename` = '" .  $this->db->escape($data['filename']) . "' WHERE banner_id = '" . (int)$banner_id . "'");
	...
	public function getBanner($banner_id) {$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "banner WHERE banner_id = '" . (int)$banner_id . "'");return $query->row;}
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			foreach ($banner_image_description_query->rows as $banner_image_description) {			
				$banner_image_description_data[$banner_image_description['language_id']] = array(
					...
					'html_desc' => $banner_image_description['html_desc']
				);
			}

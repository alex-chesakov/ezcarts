...
class ControllerUserUserPermission extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "user WHERE user_group_id = '" . (int)$result['user_group_id'] . "'");
		
			$action = array();
			$action[] = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('user/user', 'token=' . $this->session->data['token'] . '&user_group_id=' . $result['user_group_id'] . $url, 'SSL')
			);	
		...
		$this->data['count_customers'] = $query->row['total'];
		$this->data['link_customers'] = $this->url->link('sale/customer', 'token=' . $this->session->data['token']);		

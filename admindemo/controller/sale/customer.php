...
class ControllerSaleCustomer extends Controller { 
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$sql= "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$result['customer_id'] . "'";
			$count_orders = $this->db->query($sql);
			
			$sql= "SELECT SUM(total) AS sumtotal FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$result['customer_id'] . "'";
			$sum_orders = $this->db->query($sql);
			
			$date_added = '';
			$sql= "SELECT date_added FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$result['customer_id'] . "' ORDER BY date_added DESC LIMIT 1";
			$last_order = $this->db->query($sql);
			if($last_order->num_rows){
				$date_added = date($this->language->get('date_format_short'), strtotime($last_order->row['date_added']));
			}
			...
			$this->data['customers'][] = array(
				...
				'count_orders' => $count_orders->row['total'],
				'sum_orders' => $sum_orders->row['sumtotal'],
...
$this->data['entry_firstname'] = $this->language->get('entry_firstname');
		...
		$this->data['count_orders'] = 0;
		$this->data['sum_orders'] = 0;
		if (!empty($customer_info)) {
			$sql= "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$customer_info['customer_id'] . "'";
			$count_orders = $this->db->query($sql);
			if($count_orders->num_rows){
				$this->data['count_orders'] = $count_orders->row['total'];
			}
			$sql= "SELECT SUM(total) AS sumtotal FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$customer_info['customer_id'] . "'";
			$sum_orders = $this->db->query($sql);
			if($sum_orders->num_rows){
				$this->data['sum_orders'] = $sum_orders->row['sumtotal'];
			}
			//	last_order
			$sql= "SELECT date_added FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$customer_info['customer_id'] . "' ORDER BY date_added DESC LIMIT 1";
			$last_order = $this->db->query($sql);
			if($last_order->num_rows){
				$this->data['date_added'] = date($this->language->get('date_format_short'), strtotime($last_order->row['date_added']));
			}
		}
		...
		if (isset($this->request->post['firstname'])) {$this->data['firstname'] = $this->request->post['firstname'];} elseif (!empty($customer_info)) { $this->data['firstname'] = $customer_info['firstname'];} else {$this->data['firstname'] = '';}
	...
	public function login() {
		...
		if ($customer_info) {
			...
			if ($store_info) {
				$this->redirect($store_info['url'] . 'index.php?route=login&token=' . $token);
			} else { 
				$this->redirect(HTTP_CATALOG . 'index.php?route=login&token=' . $token);
			}
		} else {

...
class ControllerProduct extends Controller {
	public function index() {
		...
		if(!empty($this->customer->getLocation()) and !empty($this->session->data['kitchen_id']) and !empty($this->request->get['product_id'])){
			...
			if($query->num_rows){
				...
				if ($query->row['image'] and file_exists(DIR_IMAGE  . $query->row['image'])) {
				...
				$query_wish = $this->db->query("SELECT wish_id FROM " . DB_PREFIX . "wish WHERE customer_id = '" . (int)$this->customer->getId() . "' AND product_id = '" . (int)$query->row['product_id'] . "'");
				if($query_wish->num_rows){
					$this->data['wish_active'] = true;
				}else{
					$this->data['wish_active'] = false;
				}
				...
				if($query_images->num_rows){
					foreach($query_images->rows as $row){
						if (file_exists(DIR_IMAGE . $row['image'])) {
							...
							$this->data['product_images'][] = $this->model_tool_image->resize('no_image.jpg', $this->config->get('config_image_popup_width'), $this->config->get('config_image_popup_height'));
						}
					}
				}
				...
				if($query_similar->num_rows){
					foreach($query_similar->rows as $result){
						...
						$sql = "SELECT * FROM " . DB_PREFIX . "category WHERE category_id = '" . (int)$query->row['category_id'] . "' ";
	...
	public function wish() {
		...
		if(!empty($this->request->get['product_id'])){
			$this->request->post['product'] = $this->request->get['product_id'];
		}
		if(!empty($this->request->post['product']) and $this->customer->isLogged()){
			$query = $this->db->query("SELECT wish_id FROM " . DB_PREFIX . "wish WHERE customer_id = '" . (int)$this->customer->getId() . "' AND product_id = '" . (int)$this->request->post['product'] . "'");
			if($query->num_rows){
				$this->db->query("DELETE FROM " . DB_PREFIX . "wish WHERE customer_id = '" . (int)$this->customer->getId() . "' AND product_id = '" . (int)$this->request->post['product'] . "'");
				$json['success']= 'Removed from favorites';
			}else{
				$this->db->query("INSERT INTO " . DB_PREFIX . "wish SET customer_id = '" . (int)$this->customer->getId() . "', product_id = '" . (int)$this->request->post['product'] . "'");
				$json['success']= 'Added to favorites';
			}
		}

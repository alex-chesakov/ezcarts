...
class ControllerPaymentMethod extends Controller {
	public function index() {
		...
		if (!empty($this->session->data['kitchen_id'])) {
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					...
					$method = $this->{'model_payment_' . $result['code']}->getMethod($this->session->data['kitchen_id'], $total);
					if ($method) {
		...
		if (empty($this->session->data['payment_methods'])) {
			$this->data['error_warning'] = sprintf($this->language->get('error_no_payment'), $this->url->link('profile'));
		} else {
		...
		$this->template = 'default/template/payment_method.tpl';

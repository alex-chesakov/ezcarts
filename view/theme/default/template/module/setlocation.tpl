<button class="justify-center whitespace-nowrap rounded-md ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 flex items-center gap-2 text-sm font-normal" type="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="radix-:r0:" data-state="closed" id="select_location">
...
$('#select_location').bind('click', function() {
	...
	html+= '<button class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 search">Search</button>';
	html+= '</div><div class="mt-4 space-y-2 load">';
	<?php foreach($locations as $loc){ ?>
		html+= '<button class="inline-flex items-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 w-full justify-start select_location" data-v="<?php echo $loc;?>"><?php echo $loc;?></button>';
	<?php };?>
	html+= '</div>';
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['find']) {
					...
					$('#location_modal .load').prepend('<button class="inline-flex items-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 border border-input hover:text-accent-foreground h-10 px-4 py-2 w-full justify-start bg-blue-50 hover:bg-blue-100 select_location find" data-v="' + json['find'] + '">' + json['find'] + ' (Best Match)</button>');
				}
			}
		});
	});
	...
	$('#location_modal .select_location').live("click",function(){
		$.ajax({
			url: 'index.php?route=module/setlocation/ajax&location=' + encodeURIComponent($(this).attr('data-v')),
			dataType: 'json',
			beforeSend: function() {
			},
			complete: function() {

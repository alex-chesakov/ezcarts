...
<?php /*	ТИП ЗАГОЛОВКА 1 => не авторизован и не в account */	?>
...
<span class="text-white font-black-han">MY</span>
					...
					<div class="flex items-center gap-4">
						...
						<?php /*if($logged){ ?>
							<a href="<?php echo !empty($register)?$register:'/';?>" class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 h-10 px-4 py-2 bg-[#022785] text-white hover:bg-[#022785]/90">
Sign in
							</a>
						<?php };*/ ?>
		...
		<?php };?>
<?php /*	ТИП ЗАГОЛОВКА 0 => не авторизован и в account */	?>
...
?>
		...
		<div class="flex items-center gap-4">
			<div class="flex items-center gap-2">
				<span class="text-sm text-gray-600">Shopping for:</span>
				<div class="relative kitchen_select_block">
					<button type="button" role="combobox" aria-controls="radix-:r3:" aria-expanded="false" aria-autocomplete="none" dir="ltr" data-state="closed" data-placeholder="" class="flex h-10 items-center justify-between rounded-md border border-input bg-white px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none  focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&amp;>span]:line-clamp-1 w-[200px] bar_kitchens">
						<span style="pointer-events: none;"><?php if(empty($my_kitchens_select)){ 
?>Select kitchen<?php 
...
<div class="dropdown sel_menu_kitchens">
	<?php if(!empty($my_kitchens)){ ?>
		<?php foreach($my_kitchens as $kitchen_id=>$kit){ ?>
			<label><input type="radio" name="kitchen" value="<?php echo $kit['kitchen_id'];?>" <?php if(!empty($my_kitchens_select) and $my_kitchens_select['kitchen_id'] == $kit['kitchen_id']){ echo 'checked';};?>/><span></span><div class="name_address"><?php echo $kit['name'];?><div class="address"><?php echo $kit['address'];?></div></div></label>
		<?php };?>
	<?php };?>
...
<span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center " id="total_cart"><?php echo $total_cart;?></span>
			</button>
			<button class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2" id="button_open_right_menu">
...
$(document).ready(function() {
	$('.bar_kitchens').on("click",function(){
		$('.sel_menu_kitchens').toggleClass('open');
...
//--></script>
		<?php };?>
<?php /*	ТИП ЗАГОЛОВКА 3 => без ничего */	?>
